...
@section('title', 'Tambah Produk - Mini Market')

@section('content')
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <h6 class="fw-semibold mb-0">Tambah Produk</h6>
    ...
    <div class="card h-100 p-0 radius-12">
        <div class="card-body p-24">
            <form action="{{ route('admin-mini-market.products.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row gy-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control radius-8" id="name" name="name" placeholder="Masukkan nama produk" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control radius-8" id="price" name="price" placeholder="0" value="{{ old('price') }}" required min="0">
                        @error('price')
                            <div class="text-danger-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        <label for="stock" class="form-label fw-semibold text-primary-light text-sm mb-8">Stok Awal <span class="text-danger-600">*</span></label>
                        <input type="number" class="form-control radius-8" id="stock" name="stock" placeholder="0" value="{{ old('stock') }}" required min="0">
                        @error('stock')
                            <div class="text-danger-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control radius-8" id="description" name="description" rows="3" placeholder="Masukkan deskripsi produk">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="text-danger-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        <label for="image" class="form-label fw-semibold text-primary-light text-sm mb-8">Gambar Produk</label>
                        <input type="file" class="form-control radius-8" id="image" name="image" accept="image/*">
                        <div class="text-sm">Format: jpg, png, gif. Maks: 2MB.</div>
                        @error('image')
                            <div class="text-danger-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary-600 radius-8 px-20 py-11">Simpan Produk</button>
                    </div>
                </div>
            </form>
        </div>
